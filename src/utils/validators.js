export const validateInputsWCField = (matchDetails) => {
  ...
  if (
    matchDetails.homeTeam.toLowerCase() === matchDetails.awayTeam.toLowerCase()
  )
    errors.sameteams = 'players should be different';
...
export const validateScoreWCfields = (field, value) => {
  ...
  if (!Number.isInteger(parseInt(value)))
    errors[field] = 'it should be a integer number';
...
export const validateCountries = (value, countries) => {
  ...
  if (!countries.includes(value.toLowerCase()))
    errors.countryname = 'should be a valid country name';
